...
class ArticleHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        session = getSession()
        aa = Article(title, content, datetime.datetime.now(), priority, tag)
        if pas and content:
            session.add(aa)
            session.commit()
    ...
    def get(self):
        ...
        if a:
            for val in a:
                ...
                content = re.sub(r'</?\w+[^>]*>', '', val.content)           # 替换副文本内的HTML标签
                ET.SubElement(art, 'content').text = content[0:200]         # 截取文本

        self.set_header('Content-Type', 'text/xml; charset=utf-8')
        self.write(ET.tostring(root, encoding='UTF-8'))

# 按优先级列文章
class PriorityArticleHandler(tornado.web.RequestHandler):
    __metaclass__ = meta.HandlerMetaClass
    route = r'/openerp/priorityarticle/(.*)'
    ...
    def get(self, pid):
        ...
        pagesize = int(self.get_argument('pagesize', 10))
        pagenumber = int(self.get_argument('pagenumber', 0))
        a = session.query(Article).filter(Article.priority == int(pid)).all()#offset(pagesize*pagenumber).limit(pagesize)
        if a:
            for val in a:
                art = ET.SubElement(root, 'article')
                art.attrib['id'] = str(val.id)       # 设置根节点属性
...
class CommentHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        content = root.find('content').text
        session = getSession()
        cc = Comment(articleid, content, datetime.datetime.now(), email)
        if self.emzz(email):
            session.add(cc)
            session.commit()
            self.set_header('Content-Type', 'text/xml; charset=utf-8')
...
class CommentPageHandler(tornado.web.RequestHandler):
    ...
    def get(self, aid):
        session = getSession()
        aa = session.query(Comment).filter(Comment.article_id == int(aid)).count() # 拿到某篇博客评论的总记录数
        self.set_header('Content-Type', 'text/xml; charset=utf-8')
        self.write('<count>%s</count>' %int(aa))

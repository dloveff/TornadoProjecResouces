...
class ModuleListHandler(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        for k, v in meta.ModuleMetaClass.modules.items():
            self.write('<module><name>%s</name><version>%s</version><class>%s</class><page>/%s</page></module>' % (v.__module_name__, v.__module_version__, k, v.__module_template__))
        self.write('</modules>')
...
class TemplateHandler(meta.RequestHandlerBase):
    def prepare(self):
...
class TestHandler(meta.RequestHandlerBase):
    __route__ = '/testso'
...
handlers = meta.HandlerMetaClass.handlers + [
    ...
    (r'/(fonts/.*)', tornado.web.StaticFileHandler, {'path': settings['static_path']}),
